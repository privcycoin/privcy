...
Value getworkex(const Array& params, bool fHelp)
{
 ...
 if (vNodes.empty())
 throw JSONRPCError(-9, "PRiVCY is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "PRiVCY is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
     ...
     if (vNodes.empty())
         throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PRiVCY is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PRiVCY is downloading blocks...");
