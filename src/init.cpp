...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("PRiVCY version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  PRiVCYd [options]                     " + "\n" +
                  "  PRiVCYd [options] <command> [params]  " + _("Send command to -server or PRiVCYd") + "\n" +
                  "  PRiVCYd [options] help                " + _("List commands") + "\n" +
                  "  PRiVCYd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "PRiVCY:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("PRiVCY"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("PRiVCY"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: PRiVCY.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: PRiVCYd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 17770 or testnet: 27770)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18780 or testnet: 28780)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-proxy")) {
        // to protect privcy, do not listen by default if a proxy server is specified
        SoftSetBoolArg("-listen", false);
    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  PRiVCY is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "PRiVCY server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("PRiVCY"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of PRiVCY") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart PRiVCY to complete") << "\n";
