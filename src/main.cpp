...
const string strMessageMagic = "Privcy Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Privcy: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Privcy: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	else if (nHeight >= 50 && nHeight <= 1000)
	{
		nSubsidy = 10 * COIN;
		return nSubsidy + nFees;
	}
...
// Privcy: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Privcy: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Privcy: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Privcy: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Privcy: track money supply and mint amount info
...
// Privcy: total coin age spent in transaction, in the unit of coin-days.
...
// Privcy: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Privcy: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Privcy: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Privcy: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Privcy: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Privcy: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Privcy: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Privcy: getblocks may not obtain the ancestor block rejected
    ...
    // Privcy: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Privcy", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
		...
		const char* pszTimestamp = "Privacy Coin, Take back ownership!";
        CTransaction txNew;
        txNew.nTime = 1525107655;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1525107655;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 108735;

        if(fTestNet)
        {
        	block.nTime    = 1525180252;
            block.nNonce   = 612004;
        }
        ...
        //Check
		assert(block.hashMerkleRoot == uint256("0xce0d6be3ad69da743e407df3eb6574f4a627268cdb1bb98013846e82fc24b384"));
		block.print();
	   ...
	   assert(block.CheckBlock());
        ...
        // Privcy: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Privcy: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Privcy: tell downloading node about the latest block if it's
    ...
    if (lockMain) {
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH(const CInv& inv, pto->vInventoryToSend)
            {
                ...
                // trickle out tx inv to protect privcy
